<?php
    ...
    if ($postId > 0) {
        $stmt = $pdo->prepare("SELECT p.id AS post_id, p.title, p.description, p.status, p.created_at, pi.file_path, pi.position FROM posts p LEFT JOIN post_images pi ON pi.post_id = p.id WHERE p.id = :pid AND p.user_id = :uid ORDER BY pi.position ASC");
